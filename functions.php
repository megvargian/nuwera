...
// Change download button text from product title to "Download" in my-account downloads page
add_filter('woocommerce_account_downloads_column_download-actions', 'change_download_button_text', 10, 2);
function change_download_button_text($download_link, $download) {
    // Replace the product name with "Download" in the link text
    $download_link = preg_replace('/>(.*?)<\/a>/', '>Download</a>', $download_link);
    return $download_link;
}

// Change download button text in order thank you page downloads
add_filter('woocommerce_order_downloads_table', 'change_order_downloads_table_text', 10, 2);
function change_order_downloads_table_text($downloads, $order) {
    foreach ($downloads as $key => $download) {
        // Keep the same download data but will be handled by display filter
    }
    return $downloads;
}

// Filter download file column content
add_filter('woocommerce_account_downloads_column_download-file', 'change_download_file_text', 10, 2);
function change_download_file_text($download_file, $download) {
    // Change the download file link text to just "Download"
    if (isset($download['download_url']) && isset($download['download_name'])) {
        $download_file = '<a href="' . esc_url($download['download_url']) . '" class="woocommerce-MyAccount-downloads-file button alt">Download</a>';
    }
    return $download_file;
}

// Change download button text in order received/thank you page
add_action('init', 'remove_default_order_downloads_display');
function remove_default_order_downloads_display() {
    remove_action('woocommerce_order_details_after_order_table', array(WC()->frontend_includes(), 'order_downloads'));
}

add_action('woocommerce_order_details_after_order_table', 'custom_order_downloads_display', 5);
function custom_order_downloads_display($order) {
    ...
    echo '<h2 class="woocommerce-order-downloads__title">' . esc_html__('Downloads', 'woocommerce') . '</h2>';
    echo '<table class="woocommerce-table woocommerce-table--order-downloads shop_table shop_table_responsive order_details">';
    echo '<thead><tr>';
    echo '<th class="download-product"><span class="nobr">' . esc_html__('Product', 'woocommerce') . '</span></th>';
    echo '<th class="download-remaining"><span class="nobr">' . esc_html__('Downloads remaining', 'woocommerce') . '</span></th>';
    echo '<th class="download-expires"><span class="nobr">' . esc_html__('Expires', 'woocommerce') . '</span></th>';
    echo '<th class="download-file"><span class="nobr">' . esc_html__('Download', 'woocommerce') . '</span></th>';
    echo '</tr></thead>';
    echo '<tbody>';

    foreach ($downloads as $download) {
        echo '<tr>';
        echo '<td class="download-product" data-title="' . esc_attr__('Product', 'woocommerce') . '">' . esc_html($download['product_name']) . '</td>';
        echo '<td class="download-remaining" data-title="' . esc_attr__('Downloads remaining', 'woocommerce') . '">';
        echo $download['downloads_remaining'] ? esc_html($download['downloads_remaining']) : esc_html__('&infin;', 'woocommerce');
        echo '</td>';
        echo '<td class="download-expires" data-title="' . esc_attr__('Expires', 'woocommerce') . '">';
        echo $download['access_expires'] ? esc_html(date_i18n(get_option('date_format'), strtotime($download['access_expires']))) : esc_html__('Never', 'woocommerce');
        echo '</td>';
        echo '<td class="download-file" data-title="' . esc_attr__('Download', 'woocommerce') . '">';
